(function () {
	...
	var D__testcafe_node_modules_pinkie = Promise;
	...
	var delay = function (ms) { return new D__testcafe_node_modules_pinkie(function (resolve) { return setTimeout(resolve, ms); }); };
	...
	function fetchPageToCache(pageUrl, createXHR) {
	    ...
	    var retryRequest = function () { return delay(FETCH_PAGE_TO_CACHE_RETRY_DELAY).then(requestAttempt); };
	...
	var IdlePage = /** @class */ (function () {
	    function IdlePage(statusUrl, heartbeatUrl, initScriptUrl, options) {
	        if (options === void 0) { options = {}; }
	        this.statusUrl = statusUrl;
	        this.statusIndicator = new StatusIndicator();
	        if (options.retryTestPages)
	            enableRetryingTestPages();
	        startHeartbeat(heartbeatUrl, createXHR);
	        startInitScriptExecution(initScriptUrl, createXHR);
	        this._checkStatus();
	        document.title = '[' + document.location.toString() + ']';
	    }
	    IdlePage.prototype._checkStatus = function () {
	        var _this = this;
	        checkStatus(this.statusUrl, createXHR)
	            .then(function (_a) {
	            var command = _a.command;
	            if (command.cmd === COMMAND.idle)
	                window.setTimeout(function () { return _this._checkStatus(); }, CHECK_STATUS_DELAY);
	        })
	            .catch(function () { return _this.statusIndicator.showDisconnection(); });
	    };
	    return IdlePage;
	}());
